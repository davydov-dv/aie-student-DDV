...
|----|---------------------------------------------------------------------------|------------------|-----------------------------------------------------|
| 1  | Сервис запускается по инструкциям из `project/README.md` и работает      |      +            | README.md, раздел 4; docker run ...       |
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |      +            | src/models/predictor.py, artifacts/lgbm_anomaly_model.pkl            |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |      +            | notebooks/01_eda_wifi_traffic.ipynb, notebooks/03_model_comparison.ipynb     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |      +            | notebooks/02_...baseline.ipynb, таблица 2.1 отчёта |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            |      +            | src/data/, src/models/, src/service/ |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |      +            | Dockerfile, README.md раздел 4.2      |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов/паролей    |      +            | configs/.env.example, .gitignore  |
| 8  | Реализованы логи/наблюдаемость (хотя бы консольные логи + `/health`)     |      +            | src/service/app.py, листинг 3 отчёта |
| 9  | В `report.md` **обоснован выбор финальной модели** по результатам экспериментов |+           | Раздел 2.4 отчёта (notebooks03_model_comparison) |
| 10 | `project/README.md` и `report.md` позволяют понять сценарий демонстрации |      +            | README.md раздел 7, раздел «ЗАКЛЮЧЕНИЕ» отчёта  |
