...
class QualityFlagsResponse(BaseModel):
    """Ответ заглушки модели набора флагов качества датасета."""
    flags: dict[str, bool] | None = Field(
        default=None,
        description="Булевы флаги",
    )

class HeadResponse(BaseModel):
    headData: str | None = Field(
        default=None,
        description="n строк csv файла",
    )
# ---------- Системный эндпоинт ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает набов флагов качества данных.
    """
...
@app.post(
    ...
    tags=["head"],
    summary="Первые строки dataset",
)
async def head_from_csv(file: UploadFile = File(...), n = 5) -> HeadResponse:
    """
   Эндпоинт, который принимает CSV-файл,
   параметр n (число выводимых строк)
   и возвращает первых n строк данных.
    """
    ...
    data = df.head(int(n)).to_string(index=False)
